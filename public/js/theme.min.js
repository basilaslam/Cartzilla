...
!(function () {
  ...
  null != (r = document.querySelector('.navbar-sticky')) &&
    ...
    (function () {
      ...
      if (e.length) for (var t = 0; t < e.length; t++) lightGallery(e[t], { selector: '.gallery-item', download: !1, videojs: !0, youtubePlayerParams: { modestbranding: 1, showinfo: 0, rel: 0 }, vimeoPlayerParams: { byline: 0, portrait: 0, color: 'fe696a' } });
    })(),
    (function () {
    ...
    (function () {
      ...
      if (e.length) for (var t = 0; t < e.length; t++) lightGallery(e[t], { selector: 'this', download: !1, videojs: !0, youtubePlayerParams: { modestbranding: 1, showinfo: 0, rel: 0 }, vimeoPlayerParams: { byline: 0, portrait: 0, color: 'fe696a' } });
    })(),
    (function () {
      ...
      for (var l = document.querySelectorAll('.range-slider'), e = 0; e < l.length; e++)
        !(function (e) {
          var t = l[e].querySelector('.range-slider-ui'),
            ...
            a = { dataStartMin: parseInt(l[e].dataset.startMin, 10), dataStartMax: parseInt(l[e].dataset.startMax, 10), dataMin: parseInt(l[e].dataset.min, 10), dataMax: parseInt(l[e].dataset.max, 10), dataStep: parseInt(l[e].dataset.step, 10) },
...
let btn = document.getElementById('userProfileToggler')
btn.addEventListener('click',(el)=>{
  let profileDiv = document.getElementById('userProfile')
...
$('#signin-tab').on('submit', function (e) {
  ...
  axios.post('/login', formData).then((response) => {
    if (response.data.err) {
      document.querySelector('#loginMsg').textContent = response.data.errMessage;
...
// SignUp Function
$('#signup-tab').on('submit',  async (e) => {
 let message =  document.querySelector('#signupMsg')
 message.textContent = '';
  ...
  const headers = {
    'Content-Type': 'application/json'
          ...
          const validationResult = await axios.post('/user/validate', formData,{headers: headers})

          console.log(validationResult)

          console.log(validationResult.data.data)
          if(validationResult.data.data === true){
            console.log(validationResult.data)
  axios.post('/signup', formData).then((response) => {
    if (response.data.err) {
      document.querySelector('#signupMsg').textContent = response.data.errMessage;
      console.log(response);
    } else {
      console.log(response);
      window.location = response.data.redirect;
            ...
            message.textContent = validationResult.data.message;

          }
});


//run function on all a click
function sessionCheck(event) {
  
  console.log(sessionStatus == 'false')
  if (sessionStatus === 'false') {
    event.preventDefault()
    Toastify({
      text: "You have to log in to access this feature",
      style: {
        background: "linear-gradient(to right, #FF7D7D, #FF6464)",
      },
      offset: {
...
// Buy Page Axious
 ...
 async function sweetAlert(){
  let prodVal = document.getElementById('nft_price').value;
  let prodId = document.getElementById('nft_id').value;

  axios
    .post('/user/walletBalance', {
      firstName: 'Fred',
      lastName: 'Flintstone',
    })
    .then((response) => {
      if (response.data.balance > prodVal) {
        ...
        swalWithBootstrapButtons
          ...
          .then((result) => {
            ...
            if (result.isConfirmed) {
              axios.get(`/user/product/buy?product=${prodId}&price=${prodVal}`);
            } else if (
